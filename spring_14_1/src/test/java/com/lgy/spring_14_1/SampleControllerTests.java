...
public class SampleControllerTests {
	...
	public void testConvert() throws Exception {
		...
		String jonStr = new Gson().toJson(ticketVO);
//		Gson -> 구글에서 제공되는 것으로 객체를 문자열로 변환
		log.info("@# jonStr => " + jonStr);
		
		mockMvc.perform(post("/sample/ticket").contentType(MediaType.APPLICATION_JSON).content(jonStr)).andExpect(status().is(200));
